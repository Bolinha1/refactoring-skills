...
class OrderService {
    ...
    public function totalWithTax(float $amount): float {
        return $amount + $this->taxCalculator->calculate($amount);
    }
}
```
...
- **Less boilerplate:** No constructor, property, or import needed for a class that does almost nothing
