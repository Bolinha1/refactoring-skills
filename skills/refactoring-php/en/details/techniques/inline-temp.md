...
**BEFORE — not accepted:**
```php
public function isDiscountEligible(Order $order): bool {
    $eligible = $order->getItemCount() > 10;
    return $eligible;
}
```
...
$isEligibleForBulkDiscount = $order->getItemCount() > 50;
```
...
$customer = $this->findCustomerById($order->getCustomerId());
