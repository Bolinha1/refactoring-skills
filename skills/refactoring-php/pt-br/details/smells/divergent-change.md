# SKILL: Detectando e Refatorando Divergent Change — PHP
...
- [ ] A classe tem declarações `use` de muitos namespaces não relacionados
...
class ServiçoPedido
{
    ...
    public function buscarPorId(int $id): Pedido { /* consulta BD */ }
    public function salvar(Pedido $pedido): void { /* insert/update BD */ }

    // Razão 2: muda quando as regras de negócio mudam
    public function aplicarDesconto(Pedido $pedido): void { /* lógica de desconto */ }
    public function calcularImposto(Pedido $pedido): float { /* lógica de imposto */ }

    // Razão 3: muda quando o formato do relatório muda
    public function gerarFaturaPdf(Pedido $pedido): string { /* geração de PDF */ }
    public function gerarExportacaoCsv(Pedido $pedido): string { /* exportação CSV */ }
}
```
...
class AplicadorDesconto { public function aplicar(Pedido $p): void; }
```
