# TÉCNICA: Replace Temp with Query — PHP
...
**ANTES — não aceito:**
```php
public function preco(Order $pedido): float {
    $precoBase = $pedido->getQuantidade() * $pedido->getPrecoUnitario();
    $fatorDesconto = $precoBase > 1000 ? 0.95 : 0.98;
...
```php
public function preco(Order $pedido): float {
    return $this->precoBase($pedido) * $this->fatorDesconto($pedido);
}

private function precoBase(Order $pedido): float {
    return $pedido->getQuantidade() * $pedido->getPrecoUnitario();
}

private function fatorDesconto(Order $pedido): float {
    return $this->precoBase($pedido) > 1000 ? 0.95 : 0.98;
}
```
