# TÉCNICA: Hide Delegate — Python

## Fonte
Baseado em: https://refactoring.guru/pt-br/hide-delegate

---

## 1. Problema

Um cliente acessa um objeto delegado navegando por uma cadeia através do objeto servidor:
`cliente → servidor → delegado`. Quando o delegado muda, o cliente também precisa mudar.

---

## 2. Solução

Crie um método no servidor que esconda o delegado. O cliente agora só fala com o servidor.

---

## 3. Quando aplicar

- Um cliente chama `servidor.get_delegado().fazer_algo()`
- Uma mudança na classe delegada requer mudanças em todos os clientes que navegam até ela
- A cadeia de delegação cria o smell Message Chains
- Você quer que clientes dependam apenas da interface pública do servidor, não de sua estrutura interna

---

## 4. Passos de refatoração

1. Para cada método do delegado que clientes chamam através do servidor:
   - Crie um método delegante na classe servidor
2. Atualize o cliente para chamar o novo método do servidor em vez de navegar pela cadeia
3. Se nenhum cliente mais acessa o delegado diretamente, remova o atributo/propriedade do delegado
4. Execute os testes

---

## 5. Exemplo

**ANTES — não aceito:**
```python
class Pessoa:
    def __init__(self, departamento: "Departamento"):
        self.departamento = departamento  # exposto — clientes navegam por ele


class Departamento:
    def __init__(self, gerente: "Pessoa"):
        self.gerente = gerente


# Código do cliente — precisa conhecer Pessoa e Departamento
gerente = pessoa.departamento.gerente
```

**DEPOIS — esperado:**
```python
class Pessoa:
    def __init__(self, departamento: "Departamento"):
        self._departamento = departamento  # privado

    @property
    def gerente(self) -> "Pessoa":
        return self._departamento.gerente  # delega, esconde Departamento do cliente


# Código do cliente — só conhece Pessoa
gerente = pessoa.gerente
```

**Por que este padrão:**
- Clientes são desacoplados da existência e estrutura de `Departamento`
- Mudar como `Pessoa` encontra seu gerente é agora uma mudança em um único lugar

---

## 6. Exemplos negativos — o que NÃO fazer

**Erro 1: Adicionar a propriedade delegante mas manter o atributo público também**
```python
# Não aceito — departamento ainda é público; clientes usarão ambos os caminhos
class Pessoa:
    def __init__(self, departamento):
        self.departamento = departamento  # ainda público — deveria ser privado
    
    @property
    def gerente(self):
        return self.departamento.gerente
```

**Erro 2: Criar muitas propriedades pass-through para tudo no delegado**
```python
# Não aceito — se clientes precisam de muitas partes diferentes de Departamento,
# 10 propriedades pass-through apenas deslocam a complexidade sem reduzi-la
# (considere Remove Middle Man em vez disso)
```

**Erro 3: Não remover a cadeia após adicionar o wrapper**
```python
# Não aceito — a propriedade existe mas chamadores ainda usam a cadeia antiga
pessoa.departamento.gerente  # ainda funciona — velhos hábitos persistem
```

---

## 7. Benefícios

- **Encapsulamento:** A estrutura interna do servidor é escondida dos clientes
- **Resiliência:** Mudar a API do delegado requer apenas mudar a classe servidor
- **Simplicidade:** Clientes expressam intenção (`pessoa.gerente`) em vez de navegar pela estrutura
