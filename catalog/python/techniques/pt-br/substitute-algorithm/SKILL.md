# TÉCNICA: Substitute Algorithm — Python
...
- Um built-in do Python (`any`, `next`, operações de set, compreensões de lista) agora cobre o que você implementou manualmente
...
**ANTES — não aceito:**
```python
def encontrar_pessoa(self, pessoas: list[str]) -> str:
...
def encontrar_pessoa(self, pessoas: list[str]) -> str:
    ...
    return next((p for p in pessoas if p in candidatos), "")
```

**Por que esse padrão:**
- A busca em set (`in candidatos`) é O(1) vs. comparações de string O(n)
- Adicionar um novo candidato requer apenas uma mudança (adicionar ao set), não um novo bloco `if`
- `next(..., "")` expressa diretamente a intenção de "primeiro que corresponder ou string vazia"
...
- **Pythônico:** Usa idiomas e built-ins do Python ao invés de loops imperativos verbosos
