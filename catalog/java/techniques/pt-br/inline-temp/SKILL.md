...
1. Verifique que a variável é atribuída apenas uma vez (idealmente declare como `final`)
...
**ANTES — não aceito:**
```java
public boolean isDescontoElegivel(Pedido pedido) {
    boolean elegivel = pedido.getQuantidadeItens() > 10;
    return elegivel;
}
```
...
boolean isElegivelParaDescontoEmVolume = pedido.getQuantidadeItens() > 50;
```
...
Cliente cliente = buscarClientePorId(pedido.getClienteId());
