# TECHNIQUE: Remove Middle Man — Java
...
3. Update each caller to call the delegate directly (through whatever accessor provides it)
...
public class Person {
    ...
    public List<Person> getEmployees()   { return department.getEmployees(); }
    public String getDepartmentName()    { return department.getName(); }
    public int getDepartmentBudget()     { return department.getBudget(); }
    public Location getDepartmentLocation() { return department.getLocation(); }
}
```
...
public class Person {
    ...
    public Department getDepartment() { return department; } // expose the delegate directly
...
List<Person> team = dept.getEmployees();
```
...
public Person getManager() {
    if (department == null) return null;  // real logic — don't remove
