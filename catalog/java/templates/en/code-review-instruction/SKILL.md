...
- [ ] Are there fields that are only used by some methods, not all?
...
### Primitive Obsession
- [ ] Are there `String` (or equivalent) values representing tax ID, email, phone, ZIP, or currency?
- [ ] Are there integer or string constants simulating enumerated types?
- [ ] Are there multiple primitive parameters that always appear together?
- [ ] Are there arrays or maps with magic string keys to structure data?
...
- [ ] Is there a `switch`/`match` or `if/else` chain that dispatches by type?
