...
E.g.: "Order processing method in the service layer of an e-commerce application."]
...
- Language is [Java / Python / PHP] version [X]
...
### Context
Method `processOrder` in the `OrderService` class of a Java e-commerce system.
...
### Constraints
- The signature `processOrder(Order order)` cannot change
- Language: Java 17
...
### Context
`Customer` class in a Java CRM system. The tax ID is used as `String` throughout the system
...
- New value objects must be immutable
